...
## 6. Чтобы сайт не засыпал (бесплатный тариф)

Бесплатный Web Service на Render засыпает после ~15 минут без запросов, и следующий визит ждёт холодный старт 30–60 секунд. Варианты:

- **Внешний монитор (надёжнее).** Заведите бесплатный аккаунт на [uptimerobot.com](https://uptimerobot.com) или [cron-job.org](https://cron-job.org) и добавьте HTTP(s)-проверку `https://<ваш-адрес>.onrender.com/api/health` с интервалом 5 минут. Заодно придёт уведомление, если сайт упадёт.
- **GitHub Actions (без сторонних сервисов).** В репозитории лежит `.github/workflows/keep-alive.yml` — он пингует `/api/health` каждые 10 минут. Бесплатные минуты Actions не ограничены только на **публичных** репозиториях; на приватном используйте внешний монитор. По умолчанию воркфлоу стучится на `findbest-inx1.onrender.com` — свой адрес задайте переменной репозитория `KEEPALIVE_URL` (Settings → Secrets and variables → Actions → Variables), файл менять не нужно.
- **Платный инстанс.** Тариф Starter ($7/мес) не засыпает вообще и всё равно нужен для Persistent Disk из шага 4.
